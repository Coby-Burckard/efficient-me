...
function updatePageHome() {
  ...
  this.console.log('updating page to home'); //updating the login/logout
  ...
  logInOutButton.innerText = 'log in/sign up'; //setting correct log-in event listener

  logInOutButton.removeEventListener('click', deleteToken);
  logInOutButton.addEventListener('click', showLogIn); //updating the body
...
async function updatePagetoUser(token) {
  ...
  this.console.log('updating page to user'); //hiding create and log in

  lgf = document.querySelector('.log-in-form-body');
  lgf.classList.add('hidden');
  cuf = document.querySelector('.create-form-body');
  cuf.classList.add('hidden'); //updating the login/logout button

  const logInOutButton = document.querySelector('.log-in-or-out');
  logInOutButton.classList.remove('log-in-sign-up');
  logInOutButton.classList.add('log-out');
  logInOutButton.innerText = 'log out'; //setting correct log-out event listeners

  logInOutButton.removeEventListener('click', showLogIn);
  logInOutButton.addEventListener('click', deleteToken); //updating the body
...
function buildTabs(userData) {
  ...
  */
  ...
  while (tabBody.children.length > 1) {
  ...
  if (userData != null) {
    ...
    userData.forEach(activity => {
      const newTab = buildTab(activity); // appending the tab after the default tab
...
function buildTab(activity) {
  ...
  goalBody.appendChild(deleteTabButton); // building and appending a goal form

  const goalForm = buildGoalForm(activity.id);
  goalForm.classList.add('hidden');
  goalBody.appendChild(goalForm); // unhide goal form event listener
...
function buildGoal(goal) {
  ...
  */
  ...
  TAandGoalContainer.appendChild(goalContainer); //adding style classes
  ...
  TAandGoalContainer.id = `container-${goal.id}`; //adding event listeners

  deleteButton.addEventListener('click', handleDeleteRequest);
  TAandGoalContainer.addEventListener('click', handleGoalClickGraphing); //adding goal details to respective elements
  ...
  deleteButton.id = `delete-goal-${goal.id}`; //builds paginated time allocations with a new form at top
...
function buildGoalForm(activityID) {
  ...
  */
  ...
  formContainer.appendChild(goalForm); // adding submit form event listener
...
function buildTAList(TAs, goalID) {
  ...
  */
  ...
  TAlistOverall.classList.add('TA-list-box'); // obtaining and appending a TA form
...
function buildTA(time) {
  ...
  const hours = document.createElement('div');
  const TADeleteButton = document.createElement('a'); // adding specific details
  ...
  TADeleteButton.id = `delete-ta-${time.id}`; //adding event listeners

  TADeleteButton.addEventListener('click', handleDeleteRequest); //constructing the element
...
function buildTAform(goalID) {
  ...
  */
  ...
  taBorder.appendChild(taForm); //adding event listener to catch the submit
...
function handleUserBodyClick(event) {
  ...
  */
  ...
  if (clickedElement.classList.contains('goal-expand-button')) {
    ...
    clickedElement.classList.add('hidden'); //unhiding the collapse button
    ...
    clickedElement.classList.add('hidden'); //hiding the collapse button
...
async function newGoalSubmit(event) {
  ...
  */
  ...
  const token = document.cookie.split(';').filter(item => item.trim().startsWith('token='))[0].split('=')[1]; //collecting form information
  ...
  console.log(activityID, title, description, hours, dateTime); //sending requests
...
async function newTASubmit(event) {
  ...
  console.log(goalID, title, description, hours, dateTime); //sending requests
...
function setHTMLOnPageLoad() {
  ...
  const tokenArray = document.cookie.split(';').filter(item => item.trim().startsWith('token=')); //checks for the existence of the token cookie
...
function manageCreateUser(event) {
  ...
  logInForm = document.querySelector('.log-in-form-body');
  logInForm.classList.add('hidden'); //adding create user form
...
async function updateDBActivities(activityName) {
  ...
  */
  ...
  if (response.status != 201) {
    ...
    const defaultTab = document.getElementById('default');
    defaultTab.insertAdjacentElement('afterend', newTab); // highlighting the new tab
...
function handleDeleteRequest(event) {
  ...
  */
  ...
  const target = event.target.id.split('-'); //unhide delete pop up

  const popup = document.getElementById('delete-popup');
  popup.classList.remove('hidden'); //set event listener with existing target passed in
...
function handleDeletePopupSubmission(target) {
  return async function removeMe(event) {
    ...
    */
    event.preventDefault();
    const token = document.cookie.split(';').filter(item => item.trim().startsWith('token='))[0].split('=')[1]; // remove the delete event listener

    this.removeEventListener('submit', removeMe); //obtaining the value submitted by the user
    ...
    if (value == "yes") {
      ...
      const targetID = target[2]; // correcting target type to correspond to API url and hiding apropriate element
...
window.onload = function () {
  this.console.log('loaded page'); //setting home event listeners
  ...
  tabBody.addEventListener('click', manageTabClick); // handles goal expansion
...
function buildChartDatasets(userData) {
  ...
  */
  ...
  for (let i = 0; i < userData.length; i++) {
    ...
    const goalSet = activity.goal_set; //reseting 

    activityCumSum = 0;
    activityData = []; // looping through each goal to sum up time allocations and build goal
